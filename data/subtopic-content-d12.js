...
window.subtopicContentD12 = {
  ...
  "1.1.a": {
    info: "Routers operate at Layer 3 (Network) and make forwarding decisions based on destination IP addresses using a routing table. They connect different networks (e.g., your LAN to the internet) and segment broadcast domains. Every interface on a router is a separate broadcast domain and a separate network.",
    visual: { type: "packet-flow", params: { nodes: ["LAN A", "Router", "LAN B"], color: "#3b82f6" } },
    hack: {
      memory: "Router = post office. Reads the ZIP code (IP) and sends the letter to the right neighborhood (network).",
      practice: "In Packet Tracer, connect two PCs on different subnets via a router. Assign IPs, configure interfaces with 'ip address' and 'no shutdown', then ping across. Verify with 'show ip route'.",
      effort: "medium",
      meta: "Jeremy's IT Lab Day 8 covers routers in depth. Most students say routers click once you lab basic static routing."
    }
  },

  "1.1.b": {
    info: "Layer 2 switches forward frames based on MAC address tables and operate at the Data Link layer. Layer 3 switches add routing capability — they can route between VLANs using SVIs (Switch Virtual Interfaces) without needing an external router. L3 switches use hardware ASICs for wire-speed routing.",
    visual: {
      type: "comparison",
      params: {
        left: { label: "L2 Switch", items: ["MAC-based forwarding", "Single broadcast domain (per VLAN)", "No routing", "Cheaper"] },
        right: { label: "L3 Switch", items: ["MAC + IP forwarding", "Routes between VLANs via SVI", "Hardware-based routing", "Used at distribution layer"] }
      }
    },
    hack: {
      memory: "L2 switch = hallway receptionist who reads name tags (MAC). L3 switch = receptionist who also knows building addresses (IP) and can redirect you.",
      practice: "Build a topology with 2 VLANs on an L3 switch. Create SVIs (interface vlan 10, interface vlan 20), assign IPs, enable 'ip routing', and ping between VLANs.",
      effort: "medium",
      meta: "The L2 vs L3 distinction is a guaranteed exam topic. Understand that 'inter-VLAN routing' is the key L3 switch advantage."
    }
  },

  "1.1.c": {
    info: "Next-Generation Firewalls (NGFWs) inspect traffic up to Layer 7 (Application), combining traditional firewall rules with IPS, application awareness, URL filtering, and malware detection. An IPS (Intrusion Prevention System) sits inline and actively blocks malicious traffic, unlike an IDS which only alerts. Cisco Firepower is the primary NGFW for CCNA.",
    visual: {
      type: "layer-stack",
      params: {
        layers: ["L7 Application — Deep packet inspection", "L4 Transport — Port-based filtering", "L3 Network — IP ACLs", "L2 Data Link — MAC filtering", "L1 Physical"],
        highlight: 0
      }
    },
    hack: {
      memory: "Traditional firewall = bouncer checking IDs (ports). NGFW = bouncer + detective + drug dog — checks what's inside your bag (application data).",
      practice: "Memorize the difference: IDS = detective (detects, alerts); IPS = police (detects AND blocks). Know that NGFW = stateful firewall + IPS + app awareness.",
      effort: "low",
      meta: "The exam tests concepts, not configuration. Know IPS vs IDS, and that NGFWs operate at L7. Flashcard the difference."
    }
  },

  "1.1.d": {
    info: "Wireless Access Points (APs) provide Layer 1/Layer 2 connectivity to wireless clients by converting RF signals to Ethernet frames. Autonomous APs are standalone and configured individually. Lightweight APs are managed centrally by a Wireless LAN Controller (WLC) using CAPWAP tunnels. APs extend the wired network into the wireless domain.",
    visual: { type: "packet-flow", params: { nodes: ["Wireless Client", "AP (CAPWAP)", "WLC", "Switch"], color: "#8b5cf6" } },
    hack: {
      memory: "AP = translator between Wi-Fi language (802.11) and Ethernet language (802.3). Autonomous = solo artist. Lightweight = orchestra member (WLC is the conductor).",
      practice: "Flashcard the two AP modes. Know CAPWAP uses UDP 5246 (control) and UDP 5247 (data). In Packet Tracer, connect a lightweight AP to a WLC and observe CAPWAP registration.",
      effort: "medium",
      meta: "Wireless is 10% of the exam. Know autonomous vs lightweight and CAPWAP basics — don't over-study AP configuration."
    }
  },

  "1.1.e": {
    info: "A Wireless LAN Controller (WLC) centrally manages lightweight APs — pushing configs, firmware, RF settings, and security policies. Cisco DNA Center (now Catalyst Center) is a higher-level network management platform that provides automation, assurance, and policy through intent-based networking (IBN). DNA Center manages the entire network, not just wireless.",
    visual: {
      type: "hierarchy",
      params: {
    ...
    hack: {
      memory: "WLC = regional manager for APs. DNA Center = CEO of the entire network. WLC handles wireless; DNA Center handles everything.",
      practice: "Know what each controller does: WLC = wireless only, DNA Center = network-wide automation + assurance. Know 'intent-based networking' = you declare the desired state and the controller implements it.",
      effort: "low",
      meta: "DNA Center appears as concept questions on the exam. You won't configure it — just know what it does and that it's Cisco's SDN controller."
    }
  },

  "1.1.f": {
    info: "Endpoints are the devices that originate or consume data on a network — PCs, laptops, phones, tablets, printers, IP cameras, IoT sensors, and smart devices. They are the 'edge' of the network and the primary targets of security policies. Each endpoint has at least one NIC with a MAC and (usually) an IP address.",
    visual: { type: "packet-flow", params: { nodes: ["PC/Phone", "Switch", "Router", "Server"], color: "#10b981" } },
    hack: {
      memory: "Endpoints = the people in a conversation. Everything else (switches, routers) = the phone system connecting them.",
      practice: "Quick memorize — endpoints generate traffic, infrastructure forwards it. Know that IoT devices are also endpoints. Flashcard only.",
      effort: "low",
      meta: "This is a freebie concept on the exam. If asked 'which is an endpoint?', pick the device that generates data (PC, phone, camera), not the one that forwards it."
    }
  },

  "1.1.g": {
    info: "Servers provide services to clients — DNS, DHCP, web, email, file storage, authentication (RADIUS/TACACS+). They run specialized software and typically sit in a data center or server room. In a client-server model, the server responds to requests. The exam expects you to associate services with the correct server type and port numbers.",
    visual: { type: "packet-flow", params: { nodes: ["Client", "Switch", "Server (DNS/DHCP/Web)"], color: "#f59e0b" } },
    hack: {
      memory: "Server = waiter at a restaurant. You (client) order, the server brings the food (data). Different servers bring different dishes (DNS = directions, DHCP = table assignment, Web = the menu).",
      practice: "In Packet Tracer, set up a DHCP server and a DNS server. Have a PC request an IP via DHCP and resolve a hostname via DNS. This reinforces the client-server model.",
      effort: "low",
      meta: "Know the common server types and their ports. This is tested indirectly through protocol questions, not as a standalone topic."
    }
  },

  "1.1.h": {
    info: "Power over Ethernet (PoE) delivers DC power alongside data over standard Ethernet cables, eliminating the need for separate power adapters. 802.3af (PoE) provides up to 15.4W, 802.3at (PoE+) provides up to 30W, and 802.3bt (PoE++) provides up to 60W (Type 3) or 90W (Type 4). PoE is critical for APs, IP phones, and cameras. The PSE (Power Sourcing Equipment, usually the switch) negotiates power with the PD (Powered Device).",
    visual: {
      type: "comparison",
      params: {
        left: { label: "PoE Standards", items: ["802.3af — 15.4W", "802.3at — 30W (PoE+)", "802.3bt — 60/90W (PoE++)"] },
        right: { label: "Common PDs", items: ["IP Phones (~7W)", "Access Points (~15-25W)", "PTZ Cameras (~30-60W)"] }
      }
    },
    hack: {
      memory: "af = fifteen (15W, the 'f' in fifteen). at = thirty (30W, 't' for thirty). bt = big time (60-90W).",
      practice: "Memorize the three standards and their wattage. Flashcard: 802.3af=15.4W, 802.3at=30W, 802.3bt=60/90W. Know that APs commonly need PoE+.",
      effort: "low",
      meta: "Expect 1-2 questions on PoE wattage. The mnemonic 'af=fifteen, at=thirty' is all over r/ccna and works."
    }
  },
