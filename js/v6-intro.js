/* ═══════════════════════════════════════════════════════════════════
   ...
   Slow-feeling but quick. Eased. Self-contained CSS + DOM.
   ═══════════════════════════════════════════════════════════════════ */
(function () {
  'use strict';
  ...
  style.textContent = [
    ...
    /* Backdrop — frosted, slow-blurring in */
    ...
    /* Slow drifting amber halo */
    '#v6intro .v6-halo{position:absolute;top:18%;left:50%;width:880px;height:880px;max-width:120vw;max-height:120vw;transform:translate(-50%,-50%);background:radial-gradient(closest-side,rgba(255,191,90,.32),rgba(255,191,90,.08) 45%,transparent 70%);filter:blur(20px);animation:v6drift 11s ' + EASE + ' infinite alternate;pointer-events:none}',
    '@keyframes v6drift{from{transform:translate(-52%,-48%) scale(1)}to{transform:translate(-48%,-52%) scale(1.08)}}',

    /* Sparkle layer */
    '#v6intro .v6-spark{position:absolute;inset:0;pointer-events:none;overflow:hidden}',
    ...
    /* Card */
    '#v6intro .v6-card{position:relative;max-width:720px;width:calc(100% - 48px);padding:48px 44px 36px;border-radius:22px;background:linear-gradient(180deg,rgba(255,250,243,.97),rgba(250,243,232,.94));border:1px solid rgba(180,83,9,.18);box-shadow:0 40px 120px rgba(20,12,4,.45),0 8px 24px rgba(180,83,9,.18),inset 0 1px 0 rgba(255,255,255,.6);transform:translateY(28px) scale(.97);opacity:0;transition:transform 1s ' + EASE + ',opacity .8s ' + EASE + '}',
    '#v6intro.on .v6-card{transform:translateY(0) scale(1);opacity:1}',

    /* Top mini-tag */
    '#v6intro .v6-tag{display:inline-block;font-size:.66rem;font-weight:700;letter-spacing:.22em;text-transform:uppercase;color:#B45309;background:rgba(180,83,9,.08);border:1px solid rgba(180,83,9,.18);padding:5px 12px;border-radius:99px;margin-bottom:18px;opacity:0;transform:translateY(8px);transition:all .7s .1s ' + EASE + '}',
    '#v6intro.on .v6-tag{opacity:1;transform:translateY(0)}',

    /* Big headline — gold gradient with slow shimmer */
    '#v6intro .v6-h1{font-family:var(--font-serif,"Source Serif 4",Georgia,serif);font-size:clamp(2.4rem,6vw,3.6rem);font-weight:800;line-height:1.02;letter-spacing:-.02em;margin:0 0 6px;background:linear-gradient(95deg,#7a3a04 0%,#B45309 30%,#E89B3D 50%,#B45309 70%,#7a3a04 100%);background-size:220% 100%;-webkit-background-clip:text;background-clip:text;color:transparent;animation:v6shimmer 6.5s linear infinite;opacity:0;transform:translateY(14px) scale(.985);transition:opacity .9s .25s ' + EASE + ',transform 1.1s .25s ' + EASE + '}',
    '@keyframes v6shimmer{from{background-position:0% 0}to{background-position:220% 0}}',
    '#v6intro.on .v6-h1{opacity:1;transform:translateY(0) scale(1)}',

    /* Subtitle */
    '#v6intro .v6-sub{font-size:1rem;color:#5b4838;margin:0 0 28px;opacity:0;transform:translateY(10px);transition:all .8s .45s ' + EASE + '}',
    '#v6intro.on .v6-sub{opacity:1;transform:translateY(0)}',

    /* Section label */
    '#v6intro .v6-since{display:flex;align-items:center;gap:12px;font-size:.66rem;font-weight:700;letter-spacing:.18em;text-transform:uppercase;color:#8a6a4a;margin:0 0 14px;opacity:0;transform:translateX(-8px);transition:all .8s .55s ' + EASE + '}',
    '#v6intro.on .v6-since{opacity:1;transform:translateX(0)}',
    ...
    '#v6intro.on .v6-item{opacity:1;transform:translateX(0);filter:blur(0)}',
    ...
    /* Dismiss btn */
    '#v6intro .v6-go{display:block;margin:0 auto;font-family:inherit;font-size:.86rem;font-weight:700;color:#fff;background:linear-gradient(180deg,#C16513,#9a4a08);border:0;border-radius:99px;padding:13px 32px;cursor:pointer;letter-spacing:.04em;box-shadow:0 6px 18px rgba(180,83,9,.4),inset 0 1px 0 rgba(255,255,255,.25);opacity:0;transform:translateY(10px);transition:all .7s 1.1s ' + EASE + ',box-shadow .25s,transform .25s}',
    '#v6intro.on .v6-go{opacity:1;transform:translateY(0)}',
    ...
    /* Mobile */
    '@media (max-width:560px){#v6intro .v6-card{padding:32px 22px 26px;border-radius:18px}#v6intro .v6-h1{font-size:2.1rem}#v6intro .v6-sub{font-size:.92rem}#v6intro .v6-item{padding:11px 12px}#v6intro .v6-item-body{font-size:.78rem}}',
  ...
  var listHtml = UPDATES.map(function (u, i) {
    var delay = (0.7 + i * 0.13).toFixed(2);
  ...
  // Pre-position 14 sparkles randomly
  var sparkHtml = '';
  for (var i = 0; i < 14; i++) {
  ...
  root.innerHTML =
    ...
    '<div class="v6-card" role="document">' +
      ...
      '<h1 class="v6-h1">Version 6 is out</h1>' +
  ...
  // Slight delay so the page paints first — perceived smoothness
  setTimeout(open, 220);
})();
