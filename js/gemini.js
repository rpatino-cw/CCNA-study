...
(function () {
  ...
  function buildChatPrompt(pagesIndex) {
    const lines = [
      ...
      'tutor reply.',
      '',
      'NAVIGATION INTENT — fill `nav`:',
      'When the user uses a navigation verb ("where is", "where can I find",',
      '"take me to", "open", "go to", "show me the X page", "jump to", "navigate',
      'to", "find me the", "I want the X page"), you MUST fill `nav` with the',
      'matching page(s) AND keep `answer` short (one sentence pointing at the',
      'page). Do NOT generate inline practice problems, quizzes, or content as',
      'a substitute for navigation — the user explicitly asked to be taken to a',
      'page, so route them there.',
      '',
      'Even when the user phrasing is awkward or contains typos ("take me to the',
      'apge", "wheres subnet quiz"), recognize the navigation intent and fuzzy-',
      'match against the page list below. Typos and awkward phrasing are NEVER',
      'a reason to refuse — they are a reason to navigate.',
      '',
      'Pure concept questions ("explain VLANs", "what is OSPF", "how does STP',
      'work", "walk me through subnetting 192.168.1.0/24") leave `nav` empty',
      'and answer in `answer` as the tutor.',
      '',
      'When you do fill `nav`:',
