...
(function () {
  ...
  const MODE_PROMPTS = {
    ...
    glossary:
      `Write 10 multiple-choice questions about ATTRIBUTES, not noun-matching. Do NOT mirror the Terminology mode (definition→term). Instead each prompt is a SPECIFIC ATTRIBUTE phrase — OSI layer, port behavior, duplex, domain behavior, standard (802.3af/at/bt, 802.11), wattage class, forwarding decision criterion, table type (MAC table vs routing table vs ARP cache), or protocol (CAPWAP, LWAPP) — and the four options are 4 candidate attribute phrases (NOT 4 device names). Example stem: "Forwards frames using MAC addresses and floods unknown unicast" with options like "Operates at Layer 2, one collision domain per port, one broadcast domain across all ports" / "Operates at Layer 3, drops broadcasts by default" / etc. The 10 prompts must collectively span at least 8 of these attribute axes (layer, duplex, domain, standard, wattage, forwarding, table, protocol). Do NOT reuse the same 4-option set across questions. NEVER use bare device names (router/switch/firewall/server/AP) as options — options are always attribute phrases. At least 3 questions must use "which is INCORRECT" or "which is NOT true" stems. Cover ONLY concepts in the supplied transcript.`,
    scenario:
      `Write 10 multiple-choice scenario questions. Each prompt sets up a tiny realistic situation and asks what happens, what device is needed, or what breaks. Mix scenario types across the 10: (a) packet/frame forwarding decisions, (b) "device X fails — what happens to clients", (c) "you need to do Y — which 1.1 device picks", (d) duplex/domain consequence questions, (e) PoE budget or PSE/PD negotiation outcomes, (f) AP architecture choice (autonomous vs lightweight) for a given site size, (g) NGFW vs traditional firewall capability gap. Use realistic numbers (IPs, prefix lengths, port counts, wattage) consistent with the supplied transcript. Distractors must be plausible — wrong-but-tempting, not obviously wrong. Across the 10 questions, distractors must reference at least 12 distinct nouns from the transcript noun set (router, L2 switch, L3 switch, hub, repeater, NIC, endpoint, autonomous AP, lightweight AP, WLC, Cisco DNA Center, IPS, IDS, NGFW, traditional firewall, PSE, PD, collision domain, broadcast domain, half-duplex, full-duplex, MAC table). Do NOT reuse the same 4-option set (router/switch/firewall/server) across multiple questions.`,
    command:
...
2. Distractor variety: across the 10 questions, distractors must collectively reference at least 14 distinct nouns/concepts from the transcript. Never reuse the same 4-option set across multiple questions. If a question uses {router, switch, hub, firewall} as options, no other question in the same batch may use that same option set. Track the option pool as you write — if you've already used "router" in 3 questions, do not use it in question 4. Mode-specific override: if mode is "glossary", options must be ATTRIBUTE PHRASES not bare device names.
